...
$FileName = $SaveDate + "_ActDNB2.csv"
...
DEFINE
    VAR vStart6 = YEAR( EDATE( TODAY(), -12 ) ) * 100 + MONTH( EDATE( TODAY(), -12) )
    VAR vEnd    = YEAR( EDATE( TODAY(), -6 ) ) * 100 + MONTH( EDATE( TODAY(), - 6) )
...
VAR BaseFiltered_Last6 =
    CALCULATETABLE (
        ...
        KEEPFILTERS ( FILTER ( 'Calendar', 'Calendar'[Month Sort] >= vStart6 && 'Calendar'[Month Sort] < vEnd ) )
    )
